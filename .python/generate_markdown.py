...
def validate_language_source_inventory(root: Path) -> None:
    expected_readme = {f"lang_{code}.json" for code in LANGUAGE_CODES}
    expected_changelog = {f"lang_{code}.json" for code in LANGUAGE_CODES}
    actual_readme = {path.name for path in (root / ".readme").glob("lang_*.json")}
    actual_changelog = {path.name for path in (root / ".changelog").glob("lang_*.json")}
    require(
        actual_readme == expected_readme,
        f"README language source mismatch: missing={sorted(expected_readme - actual_readme)} "
        f"extra={sorted(actual_readme - expected_readme)}",
    )
    require(
...
def read_android_strings(path: Path) -> dict[str, str]:
    ...
    require(names == sorted(names), f"<string> elements are not sorted by name in {path}")
    return strings
...
def validate_localized_resources(root: Path, languages: dict[str, dict[str, Any]]) -> None:
    ...
    require(
        default_strings == localized_strings[ANDROID_DEFAULT_LANGUAGE],
        "values/strings.xml and values-en/strings.xml must be identical",
    )
...
def build_readme_values(
    ...
    content["placeholder_limitations"] = bullet_list(content["limitations"])
