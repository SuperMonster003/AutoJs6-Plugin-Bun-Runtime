...
* `改善` 実験用ランタイムを変更せず openat2 のディレクトリ制約を調べる 24 番目のプローブを追加: 4 KiB のネイティブ 5 環境で各 2 回 23/24. 元の 230 観測は成功するが, 新たな 10 件の失敗で相対/絶対/マジックリンク経由の設定ルート外テストデータ読み取りを 60 回記録. アサーションを緩和せず失敗を保存. クラッシュ, ハング, テスト UID の残存プロセスはなく, 起動した AVD は終了済み. ネイティブ修正は未実施
