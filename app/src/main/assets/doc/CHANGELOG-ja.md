...
* `改善` FD/SIGSYS 専用プローブを 5 項目追加: API 28/31/33/35 のネイティブ arm64 実機 4 台と API 33 ネイティブ x86_64 AVD で各 2 回とも 17/18. 子プロセスの FD 分離とシグナル検証は成功したが, 全 10 回で起動時の CLOEXEC フォールバック欠如を確認; この阻害要因を記録し, 公式ランタイムのバイト列と Android 13 の最低要件は変更なし
