...
* `优化` 不改实验运行时, 新增第 24 项 openat2 目录约束探针: 五个原生 4 KiB 环境各两轮 23/24. 原 230 项观测通过, 新增 10 次失败记录相对/绝对/魔术链接读取配置根外合成测试哨兵共 60 次. 不放宽断言, 如实归档失败门禁; 无崩溃, 挂起或测试 UID 进程残留, 本轮 AVD 已关闭. 原生修复仍未实施
