...
* `新增` 新增多檔案專案的外掛側工作區封存展開器 (M6): 有界 ZIP 快照經路徑正規化, 拒絕穿越/絕對/反斜線路徑, 大小寫與 Unicode 形式不敏感的重複偵測, 檔案/目錄衝突檢查, 條目數與位元組上限, 進入點檔案檢驗和取消清理後, 原子展開到本次執行的私有工作區, 只建立一般檔案和目錄. 已發佈的契約 v1 仍只執行單一原始碼; 封存請求鍵的提案見 docs/design/m6-workspace-archive.md
