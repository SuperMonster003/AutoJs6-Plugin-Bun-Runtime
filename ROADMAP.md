...
一句话概括方向: 插件从 "单个脚本文件的独立 Bun 引擎" (M0, 已完成) 出发, 已交付 "Android 13+ 正式基线" (M1, v0.2.3 已发布, 含监督器强制终止, 十语言错误摘要, M6 归档接收与 M7 宿主信息快照), 已把 "Android 9 至 12L 实验支持" (M3) 推进到十三补丁 baseline `1.4.0+e8b129616` 并冻结; 从 2026-09-14 起, 主线转向 "多文件项目执行" (M6) 与 "受控的 AutoJs6 能力桥" (M7). 实验线的开放根因只按下文的诊断预算处理, 不再阻塞产品线.
...
| M1 Android 13 正式基线 | v0.2.3 已发布 (2026-09-16), 覆盖升级与全新安装验收完成 | 官方 Bun 保持不变; 监督器终止, 本地化错误与 M6 归档接收已随 v0.2.2 交付, M7 宿主信息快照已随 v0.2.3 交付 | 插件/测试/设备/发布 |
