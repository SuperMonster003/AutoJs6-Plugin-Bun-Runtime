...
一句话概括方向: 插件从 "单个脚本文件的独立 Bun 引擎" (已完成) 出发, 依次走向 "Android 13 正式基线" (v0.2.0 已发布, v0.2.1 开发版已修复强制终止), "Android 9 至 12L 实验支持" (启动 CLOEXEC, spawn FD 与保留目录服务的 openat2 回退已修复; 新九补丁在五个原生 4 KiB 环境按原 24 项断言各两轮通过, 同一 APK 又在三星原生 ARM64 16 KiB 两轮通过, 完整 Binder 待验收), 以及更远的 "多文件项目执行" 与 "受控的 AutoJs6 能力桥" (未开始).
...
| 当前缺口 | 目录约束修复已通过本地五环境 240/240, 同一 ARM64 APK 又在三星原生 16 KiB 通过 48/48; 本轮设备缺口已补齐. 其余 syscall、Android 高位 FD/硬限制降低/UNSHARE、完整实验 Binder、API 29/30/32 和 patched paired APK/source Release 仍待完成; 旧失败/较窄成功记录均保留, 不将局部验收当作稳定 Android 9 支持 |
...
| M3 Android 9-12L 实验支持 | 目录回退 4 KiB / 原生 ARM64 16 KiB 已通过, 完整矩阵进行中 | 采用方案二保留普通目录服务, 九补丁五个原生 4 KiB 环境各两轮 24/24; 原 60 项越界失败全部拒绝, 同一 APK 在三星原生 16 KiB 两轮 24/24; 完整 Binder 待完成 | 插件/测试/设备/发布 |
...
- [ ] (测试) patched Bun 的两个 ABI 重复 ELF/ZIP/安装后 payload/真实执行四层门禁: 当前九补丁双 ABI 各两轮 ELF 已逐字节复现; 独立 test-only 双 ABI APK 的 ZIP/payload 验证、4 KiB 双 ABI 与原生 ARM64 16 KiB 安装后摘要和应用进程生命周期已完成. 2026-09-10 五个原生 4 KiB 环境各两轮 24/24 (240/240), 2026-09-11 同一 ARM64 APK 在 Samsung API 36 原生 16 KiB 各两轮 24/24 (48/48), 合并六环境 288/288, 不表示同轮重跑. 旧 18/20、19/20、23/24 失败及较窄成功记录均保留; 完整实验 Binder/其余 syscall/FD 语义、最终签名 APK/source 发布及 x86_64 16 KiB 仍未完成.
