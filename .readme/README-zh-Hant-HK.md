...
- `修復` 修復實驗 Android spawn 提前交付 pending SIGSYS: 父執行緒保留呼叫者 mask, 僅子路徑允許設定階段的訊號處理, 呼叫者屏蔽 SIGSYS 時使用既有子行程 cgroup 加入路徑. GCC/Clang 主機回歸涵蓋完整生產函式及舊源碼失敗對照; 新源碼建置與裝置證據獨立記錄, 不轉移歷史驗收或擴大官方支援
