...
- `Feature` Add the plugin-side workspace archive expander for multi-file projects (M6): a bounded ZIP snapshot is validated and expanded atomically into the private per-run workspace, with path normalization, rejection of traversal, absolute and backslash paths, case- and Unicode-form-insensitive duplicate detection, file/directory conflict checks, entry and byte limits, entry point validation and cancellation cleanup; only regular files and directories are created. The published contract v1 still executes a single source; the archive request keys are proposed in docs/design/m6-workspace-archive.md
