...
- `수정` 실험 Android spawn이 pending SIGSYS를 너무 일찍 전달하는 문제 수정: 부모에서 호출자 mask를 유지하고 설정 단계의 신호 처리는 자식에서만 허용. 호출자가 SIGSYS를 차단하면 기존 자식 cgroup 참여 경로 사용. GCC/Clang 호스트 회귀로 실제 함수 전체와 이전 소스의 실패 대조군을 검증하며, 새 소스의 빌드 및 기기 증거는 과거 검증 결과 및 공식 지원 범위와 별도로 기록
