...
- `개선` FD/SIGSYS 전용 프로브 5개 추가: API 28/31/33/35 네이티브 arm64 실기기 4대와 API 33 네이티브 x86_64 AVD에서 각각 두 차례 17/18 기록. 자식 프로세스 FD 격리와 신호 검증은 통과했지만 전체 10회에서 시작 시 CLOEXEC 대체 경로 누락 확인; 이 차단 사유를 보존하며 공식 런타임 바이트와 Android 13 최소 요구 사항은 변경하지 않음
