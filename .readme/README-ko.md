...
- `기능` 다중 파일 프로젝트를 위한 플러그인 측 워크스페이스 아카이브 확장기 추가 (M6): 크기가 제한된 ZIP 스냅샷을 경로 정규화, 상위 디렉터리/절대/백슬래시 경로 거부, 대소문자 및 Unicode 정규화 형식에 무관한 중복 감지, 파일/디렉터리 충돌 검사, 항목 수와 바이트 상한, 진입점 검증, 취소 시 정리를 거쳐 실행별 비공개 워크스페이스에 원자적으로 확장하며 일반 파일과 디렉터리만 생성. 공개된 계약 v1은 여전히 단일 소스만 실행; 아카이브 요청 키 제안은 docs/design/m6-workspace-archive.md 참조
