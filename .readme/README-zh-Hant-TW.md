...
- JavaScript 和 TypeScript: Bun 解析並執行單一 JS 或 TS 原始碼快照, 包括 ESM syntax 和固定 Android build 內可用的 Bun API.
...
2. 安裝與裝置 ABI 相符的 release APK. 大多數手機和平板選擇 `arm64-v8a`, 相容的模擬器或裝置選擇 `x86_64`, 不確定時選擇 `universal`.
3. 開啟 AutoJs6 插件中心並啟用 Bun Runtime. 如果新安裝插件仍維持停止狀態, 使用宿主顯示的 `啟用` 操作.
4. 在 JavaScript 或 TypeScript 檔案開頭加入獨立指令 `"bun";`, 然後照常從 AutoJs6 執行.

> 0.1 版每次請求使用 `bun run --no-install <source>` 執行一個不可變原始碼快照, 因此不會自動安裝缺少的 dependency. 將現有 Rhino 或 Node.js 專案移到 Bun 前請先閱讀以下限制.
...
- 平台: Android 13 (API 33) 或更新版本, 官方 64-bit payload 支援 `arm64-v8a` 和 baseline `x86_64`. API 31 實機上的 Bun syscall 436 `close_range` 被 app seccomp 以 `SIGSYS` 終止. AOSP Android 12 及更早版本的 app allowlist 不包含此 syscall, Android 13 (T, API 33) 已允許 raw syscall. Android 14 增加公開 bionic wrapper, 但 Bun 直接呼叫 raw syscall, 不需要 API 34 libc symbol. API 33 和 API 35 實機 runtime test 已通過. API 28 到 32 仍不受支援, 需要 patched Bun runtime 處理 seccomp trap 並通過可攜式驗證.
- 宿主 contract: AutoJs6 build 5278 或更新版本, Bun runtime contract version 1.
- 請求上限: 原始碼最多 16 MiB, stdout 和 stderr combined streaming budget 最多 8 MiB, 預設 timeout 為 60 seconds.
...
- 原始碼快照暫存在每次執行專用的 private directory. Executable 從 Android read-only native library directory 啟動, 不會複製到 writable storage 後執行.
- Repository lock 同時記錄官方 release archive 和已封裝 binary. CI 在 build 前拒絕 size, SHA-256, ELF type, machine 或 alignment 漂移.
- 插件宣告網路存取 permission, 因為受信任 Bun script 可能使用 `fetch` 等 network API. 插件不是 sandbox, 請只執行你信任的 script.
...
#### 為什麼 Bun 內沒有 AutoJs6 globals?

Bun 是獨立行程和 JavaScript 引擎, 不是 Rhino compatibility layer. 未來的宿主 bridge 必須明確公開每項 automation capability, 0.1 版刻意不提供這種 bridge.

#### Script 可以 import 另一個本機 project file 嗎?

0.1 版不可以. Contract 只傳輸一個原始碼快照, 尚未傳輸 project tree, 因此無法解析相對 project import. 單檔 ESM syntax 仍受支援.
...
大多數 Android 實體裝置使用 `arm64-v8a`. 相容模擬器或 x86_64 裝置使用 baseline `x86_64`. `universal` 同時包含兩者, 在 ABI 未知時最穩妥.
...
路線圖區分目前行為和規劃中的 project snapshot, 狹窄 AutoJs6 capability bridge, 更廣泛 Android 驗證和日後 Bun upgrade. 未勾選項目代表計畫, 不代表目前已支援.
...
- `提示` Android 13 (API 33) 現為正式最低目標; API 28 到 32 在 patched Bun runtime 通過可攜式驗證前仍不受支援
- `優化` 保留固定的官方 Bun 1.4.0 Android payload, 將支援的 Android 下限從 Android 14 (API 34) 降至 Android 13 (API 33)
...
- `提示` 首個版本每次執行一個原始碼快照, 不提供 AutoJs6 globals, Java bridge, 多檔 project 或相對 project import
...
- `新增` 在隔離的 `:bun_runtime` 插件行程中支援 explicit cancellation, 60 秒 default timeout, runtime information 和 prewarming
...
- `優化` 使用 versioned Binder contract, 透過 ParcelFileDescriptor 傳輸原始碼, 原始碼上限為 16 MiB, combined output 上限為 8 MiB
...
- `優化` 將最低版本設為 Android 14 (API 34), 因為 API 31 實機上的 Bun syscall 436 `close_range` 被 app seccomp 以 `SIGSYS` 終止. 一台 Sony API 33 裝置意外通過但不能證明可攜式支援, API 35 實機 JS 和 TS Binder 往返已通過, 較低版本需等待上游 fallback
