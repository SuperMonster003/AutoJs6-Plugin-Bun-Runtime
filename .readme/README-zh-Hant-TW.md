...
1. 在 Android 14 (API 34) 或更新版本使用 AutoJs6 build 5278 (6.8.0) 或更新版本.
...
- 平台: Android 14 (API 34) 或更新版本, 官方 64-bit payload 支援 `arm64-v8a` 和 baseline `x86_64`. API 31 實機上的 Bun syscall 436 `close_range` 被 app seccomp 以 `SIGSYS` 終止. 標準 AOSP Android 13 app allowlist 仍缺少此 syscall, API 34 才加入. 一台 Sony API 33 裝置意外通過, 但這個裝置特定結果不能證明可攜式支援. API 35 實機 JS 和 TS Binder 往返已通過. API 28 到 33 仍不在支援範圍, 需要等待上游 fallback 和可攜式驗證.
