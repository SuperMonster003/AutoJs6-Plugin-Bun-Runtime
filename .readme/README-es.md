...
- `Función` Añade el expansor de archivos de espacio de trabajo del lado del plugin para proyectos multiarchivo (M6): una instantánea ZIP acotada se valida y expande atómicamente en el espacio de trabajo privado de cada ejecución, con normalización de rutas, rechazo de rutas de escape, absolutas y con barra invertida, detección de duplicados sin distinguir mayúsculas ni forma Unicode, comprobación de conflictos archivo/directorio, límites de entradas y bytes, validación del punto de entrada y limpieza al cancelar; solo se crean archivos regulares y directorios. El contrato v1 publicado sigue ejecutando una única fuente; las claves de solicitud del archivo se proponen en docs/design/m6-workspace-archive.md
