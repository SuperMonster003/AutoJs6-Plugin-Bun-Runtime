...
- `修正` 実験版 Android の epoll 待機中にブロック済み pending シグナルが早期配送される問題を修正: 呼び出し元の mask を保持し, 既存の epoll_pwait2 除外を呼び出し箇所でも維持. GCC/Clang 回帰は修正前後の待機関数全体をコンパイルして検証. 新しい native ビルドと変更のない実機テストは別途記録し, 11 パッチ版の失敗記録と公式対応範囲を維持
