...
- `改善` release 承認を主張せずに patched runtime の対応 source input を固定: Bun base source の bytes と SHA-256, WebKit/JSC の正確な tag, commit, tree, 463,115-file inventory を検証し, 一致する license text 5 件を同梱; native 19 件, Cargo 206 件, npm 125 件の source archive も相互確認しました. 実際の source 公開と release ごとの法務 review は引き続き必要です
