...
- `修正` 実験版 Android の spawn が pending SIGSYS を早期配送する問題を修正: 親で呼び出し元の mask を保持し, 設定中のシグナル処理は子だけで有効化. SIGSYS がブロックされている場合は既存の子 cgroup 参加経路を使用. GCC/Clang のホスト回帰で本体関数全体と旧ソースの失敗対照を検証し, 新ソースのビルドと実機証拠は過去の受け入れ結果や公式対応範囲と分離して記録
