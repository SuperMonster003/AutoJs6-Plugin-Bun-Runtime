...
Оба упакованных ELF executable и все записи APK проходят проверки выравнивания 16 KB. На AVD Android 16 (API 36) со страницами 16 KB APK `arm64-v8a` прошел полный набор Binder через `libndk_translation`, но нативный payload `x86_64` завершается с кодом 134 даже на минимальном скрипте; нативный arm64 еще не проверен. Это частичное свидетельство, поэтому выпуск по-прежнему не заявляет общую поддержку 16 KB.
...
Статус 16 KB: оба ELF payload и их записи APK проходят проверки выравнивания. На AVD Android 16 (API 36) со страницами 16 KB `arm64-v8a` прошел все 5 тестов Binder через `libndk_translation`, а нативный `x86_64` завершается с кодом 134 на минимальном скрипте; нативный arm64 еще не проверен. Общая поддержка 16 KB не заявляется.
