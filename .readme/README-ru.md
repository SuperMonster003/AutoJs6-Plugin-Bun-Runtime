...
- `Улучшение` Добавить пять проверок FD/SIGSYS и подтвердить исправление запуска: четыре нативных arm64-устройства (API 28/31/33/35) и нативный x86_64 AVD с API 33 теперь дважды проходят 18/18, всего 180/180; две чистые сборки каждого ABI побайтово совпадают. Сохранить прежние отчеты с результатом 17/18, не меняя официальный runtime и минимум Android 13; полный экспериментальный Binder и нативные страницы 16 KB пока не проверены
