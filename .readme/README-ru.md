...
- `Исправление` Исправлено экспериментальное обнаружение pidfd в Android при первом асинхронном spawn с заблокированным SIGSYS: существующий резервный waiter сохраняет маску вызывающего потока и ожидающие сигналы. Runtime с десятью патчами прошел неизмененные 31 пробу (310/310) и все восемь тестов Binder (80/80) в пяти нативных средах 4 KiB. Восстановленные доказательства завершения сборки явно отмечают утрату исходных кодов выхода; первый сбой запуска ART сохранен отдельно. Проверки новых файлов на Samsung API 32/16 KiB, rebase JSC и Release остаются открытыми; официальные файлы не изменены
