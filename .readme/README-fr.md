...
- `Fonctionnalité` Ajoute l'extracteur d'archive d'espace de travail côté plugin pour les projets multi-fichiers (M6) : un instantané ZIP borné est validé puis déployé atomiquement dans l'espace de travail privé de l'exécution, avec normalisation des chemins, rejet des chemins de traversée, absolus ou à barre oblique inverse, détection des doublons insensible à la casse et à la forme Unicode, contrôle des conflits fichier/répertoire, limites d'entrées et d'octets, validation du point d'entrée et nettoyage à l'annulation ; seuls des fichiers réguliers et des répertoires sont créés. Le contrat v1 publié n'exécute toujours qu'une seule source ; les clés de requête d'archive sont proposées dans docs/design/m6-workspace-archive.md
