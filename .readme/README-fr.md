...
- `Fonctionnalité` Ajoute l'exécution de projets multi-fichiers via des archives d'espace de travail (M6) : le contrat partagé reçoit les clés de requête workspaceArchiveVersion, workspaceEntryPoint, workspaceMaxEntries et workspaceMaxBytes ainsi que la capacité SUPPORTS_WORKSPACE_ARCHIVE, et runScript déploie atomiquement un instantané ZIP borné dans l'espace de travail privé de l'exécution avant de lancer le fichier d'entrée avec le projet comme répertoire de travail. Règles : chemins relatifs uniquement, rejet des chemins de traversée, absolus, à barre oblique inverse, à deux-points ou à caractères de contrôle, détection des doublons insensible à la casse et à la forme Unicode, contrôle des conflits fichier/répertoire, au plus 16384 entrées, 64 Mio décompressés et 16 Mio par fichier, validation du point d'entrée et nettoyage à l'annulation ; seuls des fichiers réguliers et des répertoires sont créés, et les hôtes sans cette clé conservent le chemin à source unique inchangé. Nécessite un hôte qui empaquette le répertoire du projet ; la modification du moteur AutoJs6 est préparée en parallèle et pas encore publiée
- `Correctif` Lit les membres de l'archive source du projet via un nom d'archive relatif afin que la vérification exacte de la source du superviseur par le vérificateur de Release fonctionne aussi avec GNU tar sous Windows ; la CI et les ressources publiées sont inchangées
...
- `Amélioration` Ajouter quatre modes TLS/IPv6 hors ligne fixes via le service Binder de production: TLS 1.2/1.3, rejet du certificat et du nom d'hôte, HTTPS vérifié et IPv6 TCP/UDP/HTTP. Lier les certificats publics de test, les sources exactes, les APK et les UID des deux paquets; conserver les comptes existants et les limites de publication
...
- `Dépendance` Relève compileSdk à 37 afin que le plugin puisse consommer l'AAR partagé bun-runtime-api de la build 5280 de l'hôte AutoJs6, dont les métadonnées exigent le SDK de compilation 37 ; minSdk 33 et targetSdk 36 restent inchangés
