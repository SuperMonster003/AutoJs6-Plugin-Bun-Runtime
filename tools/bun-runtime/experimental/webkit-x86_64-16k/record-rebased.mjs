// Record new clean Bun builds while keeping the original JSC candidate immutable.
...
import { REBASED_HEAD, REBASED_TREE, REBASED_KIND, rebasedSourceBindings, validateRebasedCandidate } from "./rebased-common.mjs";
...
const builds = [bun1, bun2].map((directory, index) => {
    ...
    assert.equal(git("rev-parse", "HEAD"), REBASED_HEAD);
    assert.equal(git("rev-parse", "HEAD^{tree}"), REBASED_TREE);
    ...
    const { command, run, ...driver } = original;
...
const lock = { schemaVersion: 1, kind: REBASED_KIND, evidenceDate: new Date().toISOString().slice(0, 10),
    distributionReady: false, officialArtifact: false, cleanBunBuilds: 2, newWebKitBuilds: 0, reusedIndependentWebKitBuilds: 2,
    bunCommit: REBASED_HEAD, bunTree: REBASED_TREE, revision: "1.4.0+a9c76a599",
    ...
    bindings: rebasedSourceBindings(), artifact: { abi: "x86_64", ...facts(path), elf: inspectElfBuffer(readFileSync(path)) }, builds,
    limits: ["Two new independent clean Bun builds; exact prior independently built JSC libraries/config and upstream ICU reused",
        "No new WebKit/ICU compilation or device acceptance inferred from these builds", "Official, baseline ten-patch and historical nine-patch payload evidence unchanged", "Not Release acceptance"],
};
validateRebasedCandidate(lock);
const output = resolve(dirname(fileURLToPath(import.meta.url)), "rebased-candidate.lock.json");
writeFileSync(output, JSON.stringify(lock, null, 2) + "\n", { flag: "wx" });
console.log(`Rebased candidate recorded: ${lock.artifact.sha256}; two actual build-driver exits 0`);
