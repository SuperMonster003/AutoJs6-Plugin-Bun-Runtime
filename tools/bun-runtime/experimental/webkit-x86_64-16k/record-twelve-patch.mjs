// Record twelve-patch Bun builds separately from both historical JSC candidates.
...
import { TWELVE_PATCH_HEAD, TWELVE_PATCH_TREE, TWELVE_PATCH_REVISION, twelvePatchSourceBindings, validateTwelvePatchCandidate, captureApi28RecipeEncodings } from "./twelve-patch-common.mjs";
...
const builds = [bun1, bun2].map((directory, index) => {
    ...
    assert.equal(git("rev-parse", "HEAD"), TWELVE_PATCH_HEAD);
    assert.equal(git("rev-parse", "HEAD^{tree}"), TWELVE_PATCH_TREE);
...
const lock = { schemaVersion: 2, kind: REBASED_KIND, evidenceDate: new Date().toISOString().slice(0, 10),
    ...
    bunCommit: TWELVE_PATCH_HEAD, bunTree: TWELVE_PATCH_TREE, revision: TWELVE_PATCH_REVISION,
    ...
    bindings: twelvePatchSourceBindings(), artifact: { abi: "x86_64", ...facts(path), elf: inspectElfBuffer(readFileSync(path)) }, builds,
    limits: ["Two new independent clean Bun builds; exact prior independently built JSC libraries/config and upstream ICU reused",
        "No new WebKit/ICU compilation or device acceptance inferred from these builds", "Official, baseline twelve-patch and historical nine/ten-patch payload evidence unchanged", "Not Release acceptance"],
};
validateTwelvePatchCandidate(lock);
const output = resolve(dirname(fileURLToPath(import.meta.url)), "twelve-patch-candidate.lock.json");
writeFileSync(output, JSON.stringify(lock, null, 2) + "\n", { flag: "wx" });
console.log(`Twelve-patch candidate recorded: ${lock.artifact.sha256}; two actual build-driver exits 0`);
