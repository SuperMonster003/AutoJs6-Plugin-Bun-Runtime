...
class JscPressureInstrumentedTest {
    private val instrumentation = InstrumentationRegistry.getInstrumentation()
    private val context = instrumentation.targetContext

    @Test
    fun fixedJitGcWasmWorkerPressure() {
        ...
        val source = instrumentation.context.assets.open("jsc-pressure.mjs").bufferedReader().use { it.readText() }
        ...
        try {
            ...
            for (mode in listOf("jit-off", "baseline", "dfg", "ftl", "gc", "wasm", "workers")) {
                val sourceFile = File.createTempFile("jsc-pressure-", ".mjs", context.cacheDir)
                try {
                            ...
                            synchronized(output) {
                                ...
                                when (event.getString(BunRuntimeContract.KEY_EVENT_TYPE)) {
                                    ...
                                    BunRuntimeContract.EVENT_STDOUT -> {
                                        ...
                                        if (output.length + text.length > 65536) callbackError.set("Pressure output exceeded bound")
                    ...
                    val result = ParcelFileDescriptor.open(sourceFile, ParcelFileDescriptor.MODE_READ_ONLY).use { fd ->
                        runtime.runScript(Bundle().apply {
                            ...
                            putString(BunRuntimeContract.KEY_EXECUTION_ID, "jsc-pressure-${UUID.randomUUID()}")
                            putString(BunRuntimeContract.KEY_SOURCE_NAME, "jsc-pressure.mjs")
                    ...
                    assertTrue("Pressure $mode failed: ${result.getString(BunRuntimeContract.KEY_ERROR_MESSAGE)}; ${callbackError.get()}", result.getBoolean(BunRuntimeContract.KEY_SUCCEEDED))
                    ...
                    assertTrue(line.startsWith("JSC_PRESSURE_RESULT="))
                    val proof = JSONObject(line.removePrefix("JSC_PRESSURE_RESULT="))
                    assertEquals(mode, proof.getString("mode"))
                    assertEquals(pages, proof.getLong("pages"))
                    assertTrue(proof.getBoolean("passed"))
                    ...
                    instrumentation.sendStatus(0, Bundle().apply { putString("stream", "JSC_PRESSURE=" + archived.toString() + "\n") })
