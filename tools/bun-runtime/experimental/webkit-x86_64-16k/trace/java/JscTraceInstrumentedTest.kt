...
class JscTraceInstrumentedTest {
    private val instrumentation = InstrumentationRegistry.getInstrumentation()
    private val context = instrumentation.targetContext

    @Test
    fun fixedTraceInlinerControls() {
        ...
        val source = instrumentation.context.assets.open("jsc-trace.mjs").bufferedReader().use { it.readText() }
        ...
        try {
            ...
            val round = InstrumentationRegistry.getArguments().getString("diagnosticRound")!!.toInt()
            assertTrue(round == 1 || round == 2)
            val modes = if (round == 1) listOf("map-off", "map-on") else listOf("map-on", "map-off")
            for (mapMode in modes) {
                val mode = "target"
                val sourceFile = File.createTempFile("jsc-trace-", ".mjs", context.cacheDir)
                try {
                            ...
                            synchronized(output) {
                                ...
                                when (event.getString(BunRuntimeContract.KEY_EVENT_TYPE)) {
                                    ...
                                    BunRuntimeContract.EVENT_STDOUT -> {
                                        ...
                                        if (outputBytes > 65536) callbackError.set("Trace/inliner output exceeded bound")
                    ...
                    val environment = Bundle().apply {
                        putString("AUTOJS6_JSC_RESTART_MODE", mode)
                        putString("BUN_JSC_alwaysGeneratePCToCodeOriginMap", (mapMode == "map-on").toString())
                        putString("BUN_JSC_dumpOptions", "1")
                        putString("BUN_JSC_collectExtraSamplingProfilerData", "true")
                        putString("BUN_JSC_printEachDFGFTLInlineCall", "true")
                    }
                    val result = ParcelFileDescriptor.open(sourceFile, ParcelFileDescriptor.MODE_READ_ONLY).use { fd ->
                        runtime.runScript(Bundle().apply {
                            ...
                            putString(BunRuntimeContract.KEY_EXECUTION_ID, "jsc-trace-${UUID.randomUUID()}")
                            putString(BunRuntimeContract.KEY_SOURCE_NAME, "jsc-trace.mjs")
                    ...
                    assertTrue(line.startsWith("JSC_RESTART_RESULT="))
                    val proof = JSONObject(line.removePrefix("JSC_RESTART_RESULT="))
                    ...
                    val gatePassed = proof.getBoolean("targetGatePassed")
                    ...
                    val requestedEnvironment = JSONObject()
                    environment.keySet().sorted().forEach { requestedEnvironment.put(it, environment.getString(it)) }
                    val terminal = JSONObject().put("succeeded", gatePassed)
                        .put("exitCode", result.getInt(BunRuntimeContract.KEY_EXIT_CODE))
                        .put("errorCode", result.getString(BunRuntimeContract.KEY_ERROR_CODE) ?: JSONObject.NULL)
                    val record = JSONObject().put("mapMode", mapMode).put("environment", requestedEnvironment)
                        .put("compatibilityAcceptance", false).put("restart", archived).put("terminal", terminal)
                        .put("stderr", synchronized(output) { errors.toString() })
                    instrumentation.sendStatus(0, Bundle().apply { putString("stream", "JSC_TRACE=" + record.toString() + "\n") })
