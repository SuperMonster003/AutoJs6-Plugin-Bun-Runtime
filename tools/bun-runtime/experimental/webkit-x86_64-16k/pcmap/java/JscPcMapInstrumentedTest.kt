...
class JscPcMapInstrumentedTest {
    private val instrumentation = InstrumentationRegistry.getInstrumentation()
    private val context = instrumentation.targetContext

    @Test
    fun fixedPcMappingControls() {
        ...
        val source = instrumentation.context.assets.open("jsc-restart.mjs").bufferedReader().use { it.readText() }
        ...
        try {
            ...
            for (mapMode in modes) {
                val mode = "target"
                val sourceFile = File.createTempFile("jsc-restart-", ".mjs", context.cacheDir)
                try {
                            ...
                            synchronized(output) {
                                ...
                                when (event.getString(BunRuntimeContract.KEY_EVENT_TYPE)) {
                                    ...
                                    BunRuntimeContract.EVENT_STDOUT -> {
                                        ...
                                        if (outputBytes > 65536) callbackError.set("PC-map output exceeded bound")
                    ...
                    val result = ParcelFileDescriptor.open(sourceFile, ParcelFileDescriptor.MODE_READ_ONLY).use { fd ->
                        runtime.runScript(Bundle().apply {
                            ...
                            putString(BunRuntimeContract.KEY_EXECUTION_ID, "jsc-restart-${UUID.randomUUID()}")
                            putString(BunRuntimeContract.KEY_SOURCE_NAME, "jsc-restart.mjs")
                    ...
                    instrumentation.sendStatus(0, Bundle().apply { putString("stream", "JSC_PCMAP=" + record.toString() + "\n") })
