...
import { PCMAP_KIND, PCMAP_TEST, validatePcMapInstrumentation, fixtureFacts, MAP_MODES, summarizePcMapObservations } from "./pcmap-common.mjs";
...
export function validatePcMapRun(r, raw) {
    assert.equal(r.schemaVersion, 1); assert.equal(r.kind, PCMAP_KIND);
    ...
    for (const [i, round] of r.rounds.entries()) {
        ...
        assert.deepEqual(round.passedTests, [PCMAP_TEST]);
        assert.deepEqual(round.pcmap, validatePcMapInstrumentation(round.stdout, r.expected.pages, round.round));
...
export function summarizePcMapRuns(reports) {
    ...
    const observations = reports.flatMap(r => r.rounds.flatMap(round => round.pcmap));
    return { environments: 2, rounds: 4, ...summarizePcMapObservations(observations) };
}
...
function main() {
    ...
    const reports = directories.map(directory => {
        ...
        validatePcMapRun(r, r.rounds.map(round => readFileSync(join(directory, `round-${round.round}.txt`), "utf8")));
    ...
    const validation = ["pcmap-common.mjs", "archive-pcmap.mjs", "../restart/restart-common.mjs", "../../api28/binder/run-binder.mjs"]
        .map(path => ({ path: `tools/bun-runtime/experimental/webkit-x86_64-16k/pcmap/${path}`, ...facts(new URL(path, import.meta.url)) }));
    const archive = { schemaVersion: 1, kind: "jsc-pcmap-diagnostics", evidenceDate: new Date().toISOString().slice(0, 10),
