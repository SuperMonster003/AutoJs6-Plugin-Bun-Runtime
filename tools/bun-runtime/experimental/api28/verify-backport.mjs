...
export function verifyBackport(bunRepository) {
  ...
  for (const patch of downstream.patches) {
    ...
    if (patch.sourceCommit === null) {
      require(["autojs6-supply-chain", "autojs6-startup-cloexec", "autojs6-spawn-fd", "autojs6-scoped-open", "autojs6-blocked-pidfd"].includes(patch.origin), `${patch.commit}: unrecognized project-owned patch origin`);
    } else {
