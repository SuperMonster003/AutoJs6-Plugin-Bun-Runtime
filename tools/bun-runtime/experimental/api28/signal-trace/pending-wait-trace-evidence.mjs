...
export function parsePendingWaitTrace(stderr, proof) {
  assert(typeof stderr === "string" && Buffer.byteLength(stderr) < 16384 && stderr.endsWith("\n"));
  const retained = stderr.trimEnd().split("\n"), last = retained.pop();
  const contexts = retained.filter(line => line.startsWith("TRACE_USER_WAIT="));
  assert.equal(contexts.length,1,"one captured wait context required");
  const wait = JSON.parse(contexts[0].slice("TRACE_USER_WAIT=".length));
  assert.deepEqual(Object.keys(wait).sort(),["leader","maskAddress","maskRead","maskWord","syscallRegister","tid"]);
  assert.equal(wait.leader,proof.pid); assert.equal(wait.tid,proof.pid);
  assert.equal(wait.syscallRegister,22,"ARM64 epoll_pwait syscall register required");
  assert.equal(wait.maskRead,true); assert(/^[0-9a-f]{16}$/.test(wait.maskAddress));
  assert.notEqual(wait.maskAddress,"0000000000000000");
  assert.equal(wait.maskWord,"0000000000000000","actual empty pointed-to kernel sigset required");
  const lines = retained.filter(line => !line.startsWith("TRACE_USER_WAIT="));
  const contextIndex = retained.indexOf(contexts[0]);
  assert(contextIndex > 0 && retained[contextIndex-1].startsWith("TRACE_USER_SIGSYS="));
  ...
  assert(childIndex >= 0 && childIndex < userIndex,"the child close_range trap precedes wait-stage user delivery");
  ...
  return { done,events,wait };
}

export function validatePendingWaitTraceRecord(record) {
  ...
  assert.equal(build.runtimeSourceCommit,"946f082ab8ede2b7cbd6ba9fddb90463a94f0330");
  assert.deepEqual(build.payloads[build.abi]["libbun_exec.so"], { bytes:87923328,
    sha256:"5ea6914e5fd2bf16cd2ee4e87036df2cabf47804983cf3ce9942da602c0cc0e7" });
  ...
  for (const run of record.runs) {
    ...
    for (const row of r.rows) {
      ...
      const proof=parsePendingWaitFailure(row.stdout,row.mode,r.abi,r.uid);
      if (row.traced) parsePendingWaitTrace(row.stderr,proof); else assert.equal(row.stderr,"");
    }
  }
  return record;
}
