...
class RuntimeApiInstrumentedTest {
    private val instrumentation = InstrumentationRegistry.getInstrumentation()
    private val context = instrumentation.targetContext

    @Test
    fun fixedOfflineApiBoundaries() {
        ...
        try {
            ...
            for (mode in listOf("files", "dns", "tcp", "http-fetch")) {
                val sourceName = "runtime-api-$mode.mjs"
                ...
                val sourceFile = File.createTempFile("runtime-api-", ".mjs", context.cacheDir)
                try {
                            ...
                            synchronized(output) {
                                ...
                                when (event.getString(BunRuntimeContract.KEY_EVENT_TYPE)) {
                                    ...
                                    BunRuntimeContract.EVENT_STDOUT -> {
                                        ...
                                        if (outputBytes > 16384) callbackError.set("Runtime API output exceeded bound")
                    ...
                    val result = ParcelFileDescriptor.open(sourceFile, ParcelFileDescriptor.MODE_READ_ONLY).use { fd ->
                        runtime.runScript(Bundle().apply {
                            ...
                            putString(BunRuntimeContract.KEY_EXECUTION_ID, "runtime-api-${UUID.randomUUID()}")
                    ...
                    if (!succeeded) {
                        ...
                        instrumentation.sendStatus(0, Bundle().apply { putString("stream", "RUNTIME_API_FAILURE=" + failure.toString() + "\n") })
                    }
                    ...
                    assertTrue("Runtime API mode failed: $mode", succeeded)
                    assertEquals("", synchronized(output) { errors.toString() })
                    val line = captured.trim()
                    assertFalse(line.contains("\n"))
                    assertTrue(line.startsWith("RUNTIME_API_RESULT="))
                    val proof = JSONObject(line.removePrefix("RUNTIME_API_RESULT="))
                    ...
                    instrumentation.sendStatus(0, Bundle().apply { putString("stream", "RUNTIME_API=" + record.toString() + "\n") })
