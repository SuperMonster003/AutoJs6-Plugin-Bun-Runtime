...
the shared Binder API. It does not change the official plugin's API 33 minimum,
...
On 2026-09-08, Sony G8441 (API 28), Sony XQ-AT72 (API 31), Redmi 22120RN86C
(API 33) and Xiaomi 23046RP50C (API 35) each passed 10 of 12 probes in two rounds.
...
The timeout and output-limit fixtures intentionally ignore SIGTERM. They failed
the bounded `destroy()` / `destroyForcibly()` / wait gate on all four devices.
The runner exits nonzero and retains both rounds; it must not be changed to call
these results successful. Android's [Process documentation](https://developer.android.com/reference/java/lang/Process#destroyForcibly())
warns about the default delegation to `destroy()`. Local SDK sources for API
28/31/35 show that delegation without a UNIXProcess override, consistent with
these observations. The production service also uses this Java method, but this
probe is not evidence of a completed reproduction through the official Binder
contract. Establish reliable process identity and true forcible termination,
then repeat both this suite and the production Binder suite.

The [device report](../../../../../docs/compatibility/2026-09-08-m3-application-probe.json)
contains exact APK/runtime digests, build-input fingerprints, environments and
bounded results. Full plugin execution, cancellation, forced-syscall/FD-CLOEXEC
semantics, API 29/30/32, native x86_64 and native 16 KiB execution remain unproven.
...
single-ABI APKs and `probe-build.json`. Each build uses an ephemeral local RSA
...
The five bundled Bun/WebKit license texts accompany the probe. Build results
...
APK SHA-256 and the executable bytes/permissions in read-only `nativeLibraryDir`.
...
package between rounds, verifies its process is absent, repeats the suite, then
...
including on failed tests. If device connectivity is lost, inspect `cleanupError`
