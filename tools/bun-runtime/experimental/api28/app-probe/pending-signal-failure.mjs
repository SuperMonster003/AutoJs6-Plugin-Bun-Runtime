...
export function validatePendingFailureProof(proof, mode, abi, uid) {
  ...
  assert.deepEqual(proof.rows, [], "failure must occur immediately after the first spawn call");
  assert.deepEqual(proof.pending, { before: "0000000000000000", checks: [[1,0],[1,0],[0,0]],
...
export function parsePendingFailure(stdout, mode, abi, uid) {
  ...
  return validatePendingFailureProof(JSON.parse(line.slice("PENDING_SIGNAL_RESULT=".length)), mode, abi, uid);
}

export function validatePendingFailureRecord(record, rawRounds) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    for (const result of run.probes.filter(p => ids.includes(p.id))) {
      ...
      assert.deepEqual(parsePendingFailure(result.stdout, PENDING_SIGNAL_MODES[result.id], env.abi, env.uid), result.pendingSignalEvidence);
    }
