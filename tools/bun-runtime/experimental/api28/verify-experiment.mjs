...
function verifyPatchSeries(patchRoot, lock, series) {
  ...
  for (let index = 0; index < downstream.patches.length; index += 1) {
    ...
    if (index === pr.commitCount + 7) {
      ...
      requireEqual(patch.sourceCommit, null, `${label}: sourceCommit`);
      require(SHA1.test(patch.stablePatchId), `${label}: invalid stablePatchId`);
      requireSameArray(patch.affectedPaths, ["packages/bun-usockets/src/eventing/epoll_kqueue.c"], `${label}: affectedPaths`);
      requireSameArray(patch.formatPatchAdditionalOptions, ["--unified=10000"], `${label}: complete epoll source context`);
      require(typeof patch.licenseImpact === "string" && patch.licenseImpact.includes("Apache-2.0"), `${label}: preserve the uSockets Apache-2.0 license context`);
    } else if (index === pr.commitCount + 5) {
      ...
      requireEqual(patch.sourceCommit, null, `${label}: sourceCommit`);
      require(SHA1.test(patch.stablePatchId), `${label}: invalid stablePatchId`);
      requireSameArray(patch.affectedPaths, ["src/sys/linux_syscall.rs"], `${label}: affectedPaths`);
    } else if (index === pr.commitCount + 3) {
      requireEqual(patch.origin, "autojs6-scoped-open", `${label}: origin`);
      requireEqual(patch.sourceCommit, null, `${label}: sourceCommit`);
      require(SHA1.test(patch.stablePatchId), `${label}: invalid stablePatchId`);
      requireSameArray(patch.affectedPaths,
        ["src/jsc/bindings/bun-scoped-open.h", "src/jsc/bindings/c-bindings.cpp", "src/sys/lib.rs"], `${label}: affectedPaths`);
    } else if (index === pr.commitCount + 2) {
      requireEqual(patch.origin, "autojs6-spawn-fd", `${label}: origin`);
      requireEqual(patch.sourceCommit, null, `${label}: sourceCommit`);
      require(SHA1.test(patch.stablePatchId), `${label}: invalid stablePatchId`);
      requireSameArray(patch.affectedPaths,
        ["src/jsc/bindings/bun-spawn-fd.h", "src/jsc/bindings/bun-spawn.cpp"], `${label}: affectedPaths`);
    } else if (index === pr.commitCount + 1) {
